...
defineCheck({
  id: 'saleItems.costBackfilled',
  severity: 'info',
  label: 'Costs entered after the sale',
  affects: ['cogs.total', 'profit.gross', 'profit.grossMargin'],
  run(ctx) {
    // Backfilling a cost_price of 0 corrects missing data rather than rewriting
    // a recorded figure — but a corrected number must never be presented as an
    // original one. This is what lets the report say so.
    const w = completedSalesIn(ctx.period, ctx.scope, 's')
    const row = ctx.db
      .prepare(
