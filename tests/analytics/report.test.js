...
describe('branding', () => {
  const { buildFooterTemplate } = electronModule('analytics/render/pdf/toPdf.js')
  const { BRAND, stockaMark, stockaMarkDataUri } = electronModule('analytics/render/html/brand.js')

  it('puts the Stocka lockup on the cover', () => {
    const doc = createDocument({
      id: 'x', title: 'Monthly Business Intelligence Report',
      period: { start: day, end: day, label: 'T' }, shop: { name: 'A Shop' }, sections: [],
    })
    const html = toHtml(doc)
    expect(html).toContain('STOCKA')
    expect(html).toContain('RETAIL MANAGEMENT SYSTEM')
    expect(html).toContain(BRAND.green)
    expect(html).toContain('brand-mark')
  })

  it('draws the mark as vector, not an embedded photograph', () => {
    // The source PNG is 808KB. Base64 would add ~1.1MB to every report and to
    // every IPC hop, and would print soft.
    const svg = stockaMark(48)
    expect(svg.startsWith('<svg')).toBe(true)
    expect(svg.length).toBeLessThan(1200)
    expect(svg).toContain(BRAND.green)
  })

  it('attributes the system in the HTML footer', () => {
    const doc = createDocument({
      id: 'x', title: 'X', period: { start: day, end: day, label: 'T' }, sections: [],
    })
    expect(toHtml(doc)).toContain('Generated by Stocka Retail Management System')
  })

  describe('the repeated page footer', () => {
    const tpl = buildFooterTemplate({ shop: { name: 'A Shop' }, period: { label: 'July 2026' } })

    it('carries the attribution and the mark', () => {
      expect(tpl).toContain('Generated by')
      expect(tpl).toContain(BRAND.name)
      expect(tpl).toContain(BRAND.system)
      // Chromium renders this in a separate document where the report's
      // stylesheet does not apply and external resources never load, so the
      // mark has to be a data URI rather than inline SVG or a file path.
      expect(tpl).toContain('data:image/svg+xml;base64,')
    })
    ...
    it('styles itself inline, because no stylesheet reaches it', () => {
      expect(tpl).toMatch(/font-size:\s*7pt/)
      expect(tpl).not.toContain('class="doc-footer"')
    })

    it('escapes the shop name', () => {
      const evil = buildFooterTemplate({ shop: { name: '<script>x</script>' }, period: { label: 'T' } })
      expect(evil).not.toContain('<script>')
      expect(evil).toContain('&lt;script&gt;')
    })

    it('produces a data URI that decodes back to valid SVG', () => {
      const uri = stockaMarkDataUri(9)
